...
const fromRecordSymptoms = (record?: DailyRecord) => {
  ...
  return keepCustomOnly((record.additionalSymptoms || []).map((entry) => ({ id: entry.id, label: entry.name, helper: 'A custom symptom you chose to keep an eye on.', iconKey: 'sparkles', source: entry.source })));
};

const fromRecordMetrics = (record?: DailyRecord) => {
  if (!record) return [] as TrackedQuantifiableMetricDefinition[];
  if (record.trackedQuantifiableMetrics?.length) return keepCustomOnly(record.trackedQuantifiableMetrics);
  if (record.quantifiableEntries?.length) return keepCustomOnly(record.quantifiableEntries.map((entry) => ({ id: entry.id, label: entry.name, helper: 'A custom lifestyle measure you want to track.', unit: entry.unit, max: entry.max || 10, min: 1, iconKey: 'activity', source: entry.source })));
  return [];
};
...
const Tracker: React.FC<TrackerProps> = ({ existingData, onSave, language, restMode = false }) => {
  ...
  const defaultSymptoms = useMemo(() => {
    ...
    const customStored = keepCustomOnly(stored || []);
    return customStored.length ? customStored : fromRecordSymptoms(latestManaged);
  }, [latestManaged]);
